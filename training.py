...
from models import GPModel, MultilayerPerceptron, CNNRegression, ConvolutionalNeuralNetwork, Autoencoder
...
def test_mlp(model, loader, args):
    ...
    for data_x, data_y in loader:
        ...
        loss_test += loss_func(out, data_y.float()).item()  # fix: 根据情况看加 .unsqueeze(1)
...
def train_mlp(model, train_loader, val_loader, optimizer, save_path, args):
    ...
    for epoch in range(args.epochs):
        ...
        for i, (data_x, data_y) in enumerate(train_loader):
            ...
            loss_func = nn.BCEWithLogitsLoss()
            # fix
            loss = loss_func(out, data_y.float())  # 根据情况看加.unsqueeze(1)
            loss.backward()
            optimizer.step()
